...
// Gmail Setup (App Password zaroori hai)
...
app.get('/', (req, res) => res.send("🚀 Master API Live!"));
...
// OTP SYSTEM (For Signup and Forget Password)
app.post('/api/send-otp', async (req, res) => {
    try {
        const { email, type } = req.body;
        const userExists = await User.findOne({ email });

        if (type === 'forget' && !userExists) return res.status(404).json({ message: "User not found" });
        ...
        await transporter.sendMail({
            ...
            html: `<h3>Hello, Your OTP is: <b style="color:#17a2b8; font-size:24px;">${otp}</b></h3>`
        });
