// ==================== EXPLICIT BRAND CRUD ENDPOINTS ====================
app.get('/brand', async (req, res) => {
    try {
        console.log('📥 GET /brand - Fetching all brands');
...
app.get('/brand/:id', async (req, res) => {
    try {
        console.log(`📥 GET /brand/:id - Fetching brand ${req.params.id}`);
        const brand = await Brand.findById(req.params.id);
        if (!brand) return res.status(404).json({ error: 'Brand not found' });
        console.log(`✅ Fetched brand: ${brand.name}`);
        res.json(brand);
    } catch (error) {
        console.error(`❌ Error fetching brand ${req.params.id}:`, error.message);
...
app.post('/brand', async (req, res) => {
    try {
        console.log('📝 POST /brand - Creating new brand:', req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Brand name is required' });
        }
        const brand = new Brand({ name: req.body.name.trim() });
        await brand.save();
        console.log(`✅ Brand created: ${brand.name}`);
        res.status(201).json(brand);
    } catch (error) {
        console.error('❌ Error creating brand:', error.message);
        res.status(400).json({ error: 'Failed to create brand', message: error.message });
    }
});

app.put('/brand/:id', async (req, res) => {
    try {
        console.log(`📝 PUT /brand/:id - Updating brand ${req.params.id}:`, req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Brand name is required' });
        }
        ...
        console.error(`❌ Error updating brand ${req.params.id}:`, error.message);
...
app.delete('/brand/:id', async (req, res) => {
    try {
        console.log(`🗑️  DELETE /brand/:id - Deleting brand ${req.params.id}`);
        const brand = await Brand.findByIdAndDelete(req.params.id);
        if (!brand) return res.status(404).json({ error: 'Brand not found' });
        console.log(`✅ Brand deleted: ${brand.name}`);
        res.json({ result: 'Done', message: 'Brand deleted successfully' });
    } catch (error) {
        console.error(`❌ Error deleting brand ${req.params.id}:`, error.message);
        res.status(500).json({ error: 'Failed to delete brand', message: error.message });
    }
});

// ==================== EXPLICIT MAINCATEGORY CRUD ENDPOINTS ====================
app.get('/maincategory', async (req, res) => {
    try {
        console.log('📥 GET /maincategory - Fetching all main categories');
        const categories = await Maincategory.find().sort({ _id: -1 });
        console.log(`✅ Fetched ${categories.length} main categories`);
        res.json(categories);
    } catch (error) {
...
app.get('/maincategory/:id', async (req, res) => {
    try {
        console.log(`📥 GET /maincategory/:id - Fetching category ${req.params.id}`);
...
app.post('/maincategory', async (req, res) => {
    try {
        console.log('📝 POST /maincategory - Creating new category:', req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Category name is required' });
        }
        const category = new Maincategory({ name: req.body.name.trim() });
        await category.save();
...
app.put('/maincategory/:id', async (req, res) => {
    try {
        console.log(`📝 PUT /maincategory/:id - Updating category ${req.params.id}:`, req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Category name is required' });
        }
...
app.delete('/maincategory/:id', async (req, res) => {
    try {
        console.log(`🗑️  DELETE /maincategory/:id - Deleting category ${req.params.id}`);
        const category = await Maincategory.findByIdAndDelete(req.params.id);
        if (!category) return res.status(404).json({ error: 'Category not found' });
        console.log(`✅ Category deleted: ${category.name}`);
        res.json({ result: 'Done', message: 'Category deleted successfully' });
    } catch (error) {
...
// ==================== EXPLICIT SUBCATEGORY CRUD ENDPOINTS ====================
app.get('/subcategory', async (req, res) => {
    try {
        console.log('📥 GET /subcategory - Fetching all subcategories');
...
app.get('/subcategory/:id', async (req, res) => {
    try {
        console.log(`📥 GET /subcategory/:id - Fetching subcategory ${req.params.id}`);
        const subcategory = await Subcategory.findById(req.params.id);
        if (!subcategory) return res.status(404).json({ error: 'Subcategory not found' });
        console.log(`✅ Fetched subcategory: ${subcategory.name}`);
        res.json(subcategory);
    } catch (error) {
        console.error(`❌ Error fetching subcategory ${req.params.id}:`, error.message);
...
app.post('/subcategory', async (req, res) => {
    try {
        console.log('📝 POST /subcategory - Creating new subcategory:', req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Subcategory name is required' });
        }
        const subcategory = new Subcategory({ name: req.body.name.trim() });
        await subcategory.save();
        console.log(`✅ Subcategory created: ${subcategory.name}`);
        res.status(201).json(subcategory);
    } catch (error) {
        console.error('❌ Error creating subcategory:', error.message);
        res.status(400).json({ error: 'Failed to create subcategory', message: error.message });
    }
});

app.put('/subcategory/:id', async (req, res) => {
    try {
        console.log(`📝 PUT /subcategory/:id - Updating subcategory ${req.params.id}:`, req.body);
        if (!req.body.name || req.body.name.trim().length === 0) {
            return res.status(400).json({ error: 'Subcategory name is required' });
        }
        ...
        console.error(`❌ Error updating subcategory ${req.params.id}:`, error.message);
...
app.delete('/subcategory/:id', async (req, res) => {
    try {
        console.log(`🗑️  DELETE /subcategory/:id - Deleting subcategory ${req.params.id}`);
        const subcategory = await Subcategory.findByIdAndDelete(req.params.id);
        if (!subcategory) return res.status(404).json({ error: 'Subcategory not found' });
        console.log(`✅ Subcategory deleted: ${subcategory.name}`);
        res.json({ result: 'Done', message: 'Subcategory deleted successfully' });
    } catch (error) {
        console.error(`❌ Error deleting subcategory ${req.params.id}:`, error.message);
