...
Sentry.init({
  ...
  tracesSampleRate: 0.2, // 20% sampling — reduces overhead while catching errors
...
// puppeteer removed — uses jsPDF only (saves 750MB+ RAM)
...
// Legacy HTML invoice template removed — jsPDF is used directly
...
const generateInvoicePdfBufferLegacy = async (invoiceData) => {
    ...
    // Direct jsPDF generation (puppeteer removed for performance)
    try {
        return buildFallbackPdf(invoiceData || {});
    } catch (err) {
        console.error('❌ Invoice PDF generation error:', err.message);
        throw new Error(`Invoice PDF failed: ${err.message}`);
    }
    };
...
// HTML invoice templates removed — jsPDF is used directly
