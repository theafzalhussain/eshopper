...
app.post('/api/chat', async (req, res) => {
    try {
        if (!genAI) {
            return res.status(503).json({ error: "AI service not configured.", message: "AI service is currently unavailable." });
        }

        const { message: prompt, conversationHistory: history } = req.body;
        console.log(`💬 AI Context check for: ${prompt}`);

        // 📊 DATABASE SYNC: Fetch product list
        ...
        // 🛠️ ROLE FIX: Map frontend sender ('user'/'bot') to Gemini roles ('user'/'model')
        let cleanHistory = (history || []).map(m => ({
            role: m.role === 'ai' || m.role === 'model' || m.sender === 'bot' ? 'model' : 'user',
            parts: [{ text: m.text || m.parts?.[0]?.text || "" }]
        }));

        // Ensuring history starts with USER - Gemini API requires alternating user/model turns starting with user
