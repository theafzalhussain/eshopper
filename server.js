...
} catch (e) { console.error('⚠️ Firebase Issue:', e.message); }
...
const schemaOptions = { timestamps: true, toJSON: { virtuals: true, transform: (doc, ret) => { ret.id = ret._id; delete ret._id; } } };
const User = mongoose.model('User', new mongoose.Schema({ uid: String, email: String, name: String }, schemaOptions));
const Product = mongoose.model('Product', new mongoose.Schema({ name: String, baseprice: Number, pic1: String }, schemaOptions));

app.post('/api/auth-sync', async (req, res) => {
    try {
        const { idToken, uid, email, name, pic } = req.body;
        await admin.auth().verifyIdToken(idToken);
        const user = await User.findOneAndUpdate({ uid }, { name, email, pic }, { upsert: true, new: true });
        res.json(user);
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        const result = await model.generateContent(`User style help: ${req.body.prompt}`);
        res.json({ text: result.response.text() });
    } catch (err) { res.status(500).json({ error: "AI Offline" }); }
});
...
async function start() {
    try {
        ...
        httpServer.listen(PORT, '0.0.0.0', () => console.log(`🚀 Master API Live: ${PORT}`));
