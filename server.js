...
app.get('/admin/stats', async (req, res) => {
    ...
    const rev = c.reduce((s, i) => s + (i.finalAmount || 0), 0);
    res.send({ totalUsers: u, totalProducts: p, totalOrders: c.length, totalRevenue: rev });
});

// UNIVERSAL HANDLER (CRUD)
...
setup('/contact', mongoose.model('Contact', new mongoose.Schema({ name: String, email: String, phone: String, message: String }, opts)));
setup('/newslatter', mongoose.model('Newslatter', new mongoose.Schema({ email: String }, opts)));
