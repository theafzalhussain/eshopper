...
// --- 3. EXPLICIT ROUTES (FIXED 500 CRASH) ---
...
// Update User (THE CRITICAL FIX)
app.put('/user/:id', upload, async (req, res) => {
    try {
        let upData = { ...req.body };
        if (req.files && req.files.pic) upData.pic = req.files.pic[0].path;

        // CRASH PROTECTION: Check if password exists and is not already a hash
        if (req.body.password && String(req.body.password).trim() !== "" && String(req.body.password).length < 25) {
            ...
            delete upData.password; // Don't touch password if not provided or invalid
        }
...
// Update Product (Fixed for Catalog)
...
app.listen(PORT, () => console.log(`🚀 Final Master Server Live on ${PORT}`));