...
import { MessageSquare, Trash2, Mail, Search, Phone as PhoneIcon } from 'lucide-react'
    ...
    const filteredContacts = useMemo(() => {
        ...
        if (searchTerm.trim()) {
            ...
            result = result.filter(c => 
                ...
                (c.message && c.message.toLowerCase().includes(lowerQuery))
            )
        }
        return result
    }, [contacts, searchTerm])
                ...
                <div className="container-fluid px-lg-4 py-4">
                    ...
                    >
                        <div className="lux-banner-content">
                            <div>
                                <div className="lux-eyebrow"><Mail size={14} className="mr-1"/> Comm Center</div>
                                <h1 className="lux-banner-title">Customer <span>Queries</span></h1>
                                <p className="lux-banner-sub">Manage and review contact requests from boutique customers.</p>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        filteredContacts.map((item, index) => (
                                            <tr key={item.id || item._id || index} className="lux-table-row">
                                                <td>
                                                    <strong className="color-ink">{item.name}</strong>
                                                </td>
                                                <td>
                                                    <div className="lux-contact-meta">
                                                        <div className="lux-contact-item"><Mail size={12}/> {item.email}</div>
                                                        <div className="lux-contact-item"><PhoneIcon size={12}/> {item.phone}</div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="text-right">
                                                    <div className="lux-action-cell">
                                                        <button 
                                                            ...
                                                            onClick={() => { if(window.confirm(`Delete message from ${item.name}?`)) handleDelete(item.id || item._id) }}
