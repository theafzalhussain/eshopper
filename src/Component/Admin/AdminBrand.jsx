        ...
        },
        {
            ...
            renderCell: ({ row }) => (
                <button className="btn btn-outline-danger rounded-circle p-2" 
                    onClick={() => { if(window.confirm("Sure?")) dispatch(deleteBrand({ id: row.id })) }}>
                    <Trash2 size={18} />
                </button>
            )
        },
                        ...
                        <motion.div initial={{opacity:0}} animate={{opacity:1}} className="bg-white shadow-xl rounded-2xl p-4 border-0">
                            <div className="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <Link to="/admin-add-brand" className='btn btn-info rounded-pill px-4 shadow-sm font-weight-bold'>
                                    <Plus size={18} className="mr-1"/> NEW BRAND
                                </Link>
                            </div>
                            <div style={{ height: 450, width: '100%' }}>
                                <DataGrid rows={brand || []} columns={columns} pageSize={7} disableSelectionOnClick />
                            </div>
                        </motion.div>
                    </div>
                </div>
