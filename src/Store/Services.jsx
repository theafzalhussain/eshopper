...
/**
 * --- 2. CORE API HANDLER (Optimized & Error Proof) ---
 */
async function fastAPI(endpoint, method = "GET", data = null) {
    const isFD = data instanceof FormData;
    const res = await fetch(`${BASE_URL}${endpoint}`, {
        method,
        // FormData ke liye headers browser set karta hai, JSON ke liye hume batana padta hai
    ...
    if (res.ok) {
        ...
        // Agar response empty hai toh success return karein, warna JSON parse karein
        return text ? JSON.parse(text) : { result: "Done" };
    } else {
        // Render free tier wake-up handle karne ke liye check
        const errorBody = await res.text();
        console.error("API Error Details:", errorBody);
        throw new Error(errorBody || "API Failure");
    }
}
